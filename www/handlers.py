...
@post('/api/users')
async def api_get_users(*, name, email, passwd):
    ...
    if len(users) > 0:
        raise APIError('registed:faild', 'email', 'email is already in use.')
...
async def authenticate(*, email, passwd):
    if not email:
        raise APIValueError('email', 'invalidate email')

    if not passwd:
        raise APIValueError('passwd', 'invalidate password')
    ...
    r.set_cookie(COOKIE_NAME, user2cookie(user, 86400), max_age=86400, httponly=True)
    user.passwd = '*****'
    r.content_type = 'applcation/json'
    r.body = json.dumps(user, ensure_ascii=False).encode('utf-8')
    return r
...
async def api_create_blog(request, *, name, summary, content):
    ...
    blog = Blog(user_id=request.__user__.id, user_name=request.__user__.name, user_image=request.__user__.image,
                name=name.strip(), content=content.strip(), summary=summary.strip())
    await blog.save()
    return blog
...
async def delete_blogs(request, *, id):
    check_admin(request)
    blog = await Blog.find(id)
    await blog.remove()
    return dict(id=id)
